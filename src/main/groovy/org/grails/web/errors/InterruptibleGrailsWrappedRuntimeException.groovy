...
import utils.InterruptibleCharSequence
...
class InterruptibleGrailsWrappedRuntimeException extends GrailsException {
    ...
    static final String URL_PREFIX = "/WEB-INF/grails-app/"
    ...
    InterruptibleGrailsWrappedRuntimeException(ServletContext servletContext, Throwable t) {
        ...
        if (cause instanceof MultipleCompilationErrorsException) {
            ...
            Matcher gsp = PARSE_GSP_DETAILS_STEP1.matcher(new InterruptibleCharSequence(stackTrace))
            try {
                boolean gspFound = ThreadUtils.withTimeLimit(2000) {
                    gsp.find()
                }
                if (gspFound) {
        ...
        try {
            ...
            if (getLineNumber() > -1) {
                ...
                if (!GrailsStringUtils.isBlank(url)) {
                    is = servletContext.getResourceAsStream(url)
                    LOG.debug("Attempting to display code snippet found is url " + url)
                }
                if (is == null) {
                    Resource r = null
                    try {
